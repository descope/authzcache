...
- `AUTHZCACHE_LOOKUP_CACHE_ENABLED` - Enable the lookup cache for `WhoCanAccess`/`WhatCanTargetAccess` queries; cached candidates are always re-verified against the fresh check cache (TRUE/FALSE, default: TRUE)
...
- `AUTHZCACHE_HTTP_WRITE_TIMEOUT_IN_SECONDS` - Max seconds the gateway will spend producing a response before it closes the connection (default: 25). A slow backend `check` that exceeds this makes the caller see a "socket hang up". Keep it below the calling SDK's cache-abort timeout (the Node SDK aborts the cache call at 30s) so slow-but-successful checks still return through the cache instead of falling back to the origin. Takes precedence over the generic `HTTP_GATEWAY_WRITE_TIMEOUT`, which is still honored when this is unset.
